...
- [`crafting-commits`](ai/plugin/skills/crafting-commits/SKILL.md) — commit-message writing standards: subjects and bodies that explain the problem, change, and reason accurately at each commit's tree.
- [`crafting-prs`](ai/plugin/skills/crafting-prs/SKILL.md) — PR titles and descriptions that build understanding for readers with little subject knowledge: introduce concepts and causal steps, follow review order where useful, and keep current validation limits visible.
...
- [`making-git-changes`](ai/plugin/skills/making-git-changes/SKILL.md) — execution mechanics for all git state changes (commit, amend, rebase, force-push, conflict resolution); checks commit readiness, routes to the planners when commit shape changes, and uses `crafting-commits` for subjects and bodies.
...
The skills coordinate across two git/PR clusters split by concern: atomic commits (`planning-commits` plans forward work and fix placement, `replanning-branches` takes over as the planner when reshaping a branch's already-committed history, `crafting-commits` explains each commit through its subject and body) and PR work (`crafting-prs` defines the writing standard; `maintaining-prs` keeps open PRs green, consulting that standard for prose and the atomic-commits skills for the shape of any repair). `making-git-changes` is the shared executor both clusters use for every git state change. Drafting or updating prose does not itself start maintenance or authorize history rewriting.
