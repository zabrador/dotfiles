...
#
#   sh ai/evals/claude/run-evals.sh <skill-name> [runs-per-trigger-query]
#   sh ai/evals/claude/run-evals.sh all [runs-per-trigger-query]
#
...
run_skill() {
  s=$1
  dir=$repo_root/ai/plugin/skills/$s
...
if [ "$target" = "all" ]; then
  for d in "$repo_root"/ai/plugin/skills/*/; do
