...
Two companion skills apply the plan: `making-git-changes` checks readiness and runs git operations; `crafting-commits` writes each commit's subject and body. When execution finds a non-atomic diff, this skill produces a revised decomposition. A writing-only task may suggest planning without authorizing it.
...
Produce the commit sequence up front — during implementation planning, with or without a named plan mode — then execute against it, invoking `making-git-changes` at each planned commit point (readiness checked there; messages use `crafting-commits`). Trivial changes collapse to a single-commit plan; there's no triviality threshold.
...
- **New unit** — if the change does something no existing commit's message
  ...
  its own message, executed via `making-git-changes` with messages from `crafting-commits`. Squashing it into an unrelated commit makes that commit dishonest
...
Once the plan is in place, `making-git-changes` executes it, staging hunks (`git add -p`) and committing one planned commit at a time, checking readiness there and messages with `crafting-commits`.
